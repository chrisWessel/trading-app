...
export default function PositionSizingGuide({
  ...
  const content = (
    <div className="bg-slate-900 border border-slate-800 text-slate-100 rounded-2xl shadow-2xl overflow-hidden max-w-6xl w-full mx-auto flex flex-col h-full max-h-[94vh]">
      {/* Fixed Header Bar */}
      <div className="bg-slate-950 border-b border-slate-800 p-4 sm:p-5 flex flex-wrap items-center justify-between gap-4 shrink-0">
        <div className="flex items-center gap-3">
          <div className="bg-gradient-to-br from-blue-600 to-indigo-600 p-2.5 sm:p-3 rounded-xl shadow-lg shadow-blue-900/40">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] bg-blue-950 text-blue-400 border border-blue-800 px-2 py-0.5 rounded-md font-mono font-bold hidden sm:inline">
        ...
        {isModal && onClose && (
          <button
            onClick={onClose}
            type="button"
            className="flex items-center gap-2 bg-rose-600 hover:bg-rose-500 text-white border border-rose-500 px-4 py-2 rounded-xl text-xs font-extrabold font-sans transition shadow-lg shadow-rose-950"
          >
            <X className="w-4 h-4" />
            <span>← Cancel / Back to Main Dashboard</span>
          </button>
        )}
      </div>

      {/* Fixed Main Navigation Subtabs */}
      <div className="bg-slate-950/90 border-b border-slate-800 px-4 pt-2 flex flex-wrap items-center gap-1.5 font-mono text-xs overflow-x-auto shrink-0">
        <button
        ...
        >
          ...
          <span>1. MetaTrader Order Types Guide</span>
        </button>
      ...
      <div className="bg-slate-950 border-t border-slate-800 px-4 sm:px-6 py-3 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-400 font-mono shrink-0">
        <div className="flex items-center gap-2">
          <Zap className="w-4 h-4 text-amber-400" />
          <span className="hidden sm:inline">PU Prime & MetaTrader Trading Suite • Institutional Risk Standard</span>
        </div>
        ...
        {isModal && onClose && (
          <button
            onClick={onClose}
            type="button"
            className="bg-rose-600 hover:bg-rose-500 text-white font-extrabold px-4 py-2 rounded-xl flex items-center gap-2 transition shadow-lg shadow-rose-950 font-sans"
          >
            <X className="w-4 h-4" />
            <span>← Cancel / Back to Main Dashboard</span>
          </button>
  ...
  if (isModal) {
    return (
      <div className="fixed inset-0 z-50 bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-2 sm:p-4 overflow-hidden">
        <div className="w-full max-w-6xl h-full max-h-[96vh] flex flex-col my-auto rounded-2xl overflow-hidden shadow-2xl">
          {content}
        </div>
      </div>
    );
